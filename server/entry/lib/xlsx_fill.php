 ...
 *
 * 値の型で書き分ける:
 * - 文字列 … inlineStr（「○1」「補」など）
 * - int / float … 数値セル。**集計に使う数はこちらで渡すこと。**文字列で書くと
 *   Excel の SUM/COUNTA が数えず、合計欄が 0 のままになる（実際に起きた）
 */
final class XlsxFill
{
    ...
    private static function applyCells(DOMDocument $doc, array $cells): array
    {
        ...
        foreach ($cells as $ref => $value) {
            ...
            self::setCellValue($doc, $row, $ref, $cn, $value);
            $written[$ref] = $value;
        }
        return $written;
    }
    ...
    private static function setCellValue(DOMDocument $doc, DOMElement $row, string $ref, int $cn, mixed $value): void
    {
        $target = null;
        $before = null;
        ...
        // 子要素を消すので、元が数式（<f>）でも計算結果の値に置き換わる
        $target->removeAttribute('t');
        while ($target->firstChild) {
            $target->removeChild($target->firstChild);
        }
        if (is_int($value) || is_float($value)) {
            $v = $doc->createElementNS(self::NS, 'v');   // t 属性なし＝数値セル
