<?php
/**
 * 申込書の型埋め: 座標(coords_*.json) ＋ 学校・部員データ → セル連想配列。
 *
 * 申込システム app.py の generate_excel の移植。**4つの大会すべてに対応する**:
 *   標準（関東予選・インターハイ予選）… 個人組手は1列。団体組手も1列
 *   階級制（新人大会）              … 個人組手は階級ごとの列。団体組手は5人制/3人制の列
 * 違いは列の探し方だけなので、階級・人数制つきの列名を先に探し、無ければ
 * 素の列名に落ちる、という1本の道で両方をまかなう。
 *
 * 共通の規則:
 * - 年度は「8年」→「8」（safe_write の末尾「年」除去と同じ）
 * - 顧問は4名まで。引率は ○/×
 * - 部員は 学年で絞り込み → display_order → 男子先 → 学年降順 → 名前順
 * - 行 = start_row + (i ÷ cap)×offset + (i mod cap)
 * - 団体 = ○/補、個人 = ○順位 / シ順位 / 補
 */
final class EntryForm
{
    /**
     * @param array $coords coords_*.json の中身
     * @param array $school year / tournament_name / date? / school_name / principal /
     *                      advisors[{name,d1,d2}] / m_kumite_mode / w_kumite_mode
     * @param array $members 部員＋エントリー（name/grade/dob/jkf_no/sex/display_order と
     *                       team_kata_chk, team_kata_role, team_kumi_chk, team_kumi_role,
     *                       kata_chk, kata_val, kata_rank, kumi_chk, kumi_val, kumi_sub_val, kumi_rank）
     * @param int[] $grades 出場できる学年
     * @return array<string,string> セル参照 => 値
     */
    public static function cells(array $coords, array $school, array $members, array $grades = [1, 2, 3]): array
    {
        $out = [];
        ...
        // 列を探す: 階級・人数制つき → 素の名前 の順。標準の表は後者しか持たない
        $col = function (array $names) use ($cols): ?int {
            foreach ($names as $n) {
                if ($n !== null && isset($cols[$n])) {
                    return (int)$cols[$n];
                }
            }
            return null;
        ...
        foreach ($list as $i => $mm) {
            ...
            if (!empty($mm['team_kumi_chk'])) {
                $mode = (string)($school["{$p}_kumite_mode"] ?? '');
                $c = $col(["{$p}_team_kumite_{$mode}", "{$p}_team_kumite"]);
                if ($c) {
            ...
            if (!empty($mm['kumi_chk'])) {
                ...
                $c = $col(["{$p}_kumite_{$weight}", "{$p}_kumite"]);
                if ($c) {
                    // 階級制は kumi_sub_val（正/シード/補）、標準は kumi_val がそのまま区分
                    $v = isset($cols["{$p}_kumite_{$weight}"])
                        ? (string)($mm['kumi_sub_val'] ?? '正')
                        : $weight;
                    $put([$r, $c], self::markWithRank($v, (string)($mm['kumi_rank'] ?? '')));
                }
            }
        }
        return $out;
